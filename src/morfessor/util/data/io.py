...
from ...util.flatcat.categorizationscheme import get_categories, CategorizedMorph
from ...models.flatcat.flatcat import FlatcatModel
from ..exception import InvalidCategoryError
from ..misc import _generator_progress, _is_string
...
# frozen copy from Morfessor Baseline
# (MorfessorIO contract is not stable enough for our needs)
class MorfessorIO(object):
    ...
    def read_segmentation_file(self, file_name: str, has_counts: bool=True, **kwargs) -> Iterator[Tuple[int, str, ]]:
        ...
        """
        _logger.info(f"Reading segmentations from '{file_name}'...")
        for line in self._read_text_file(file_name):
            if has_counts:
